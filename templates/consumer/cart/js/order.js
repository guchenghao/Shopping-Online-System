...
function showOrder() {
	...
	for (; orderJson.data.list[i]; i++) {
		...
		// div0
		$("#start").append($("<div></div>").attr({
			"id": oid + "-div0",
			"class": "panel panel-default"
		}));
		// div1
		$("#" + oid + "-div0").append($("<div></div>").attr({
			"id": oid + "-div1",
			"class": "panel-body"
		}));
		// div2
		$("#" + oid + "-div1").append($("<div></div>").attr({
			"id": oid + "-div2",
			"class": "row"
		}));
		// div3
		$("#" + oid + "-div2").append($("<div>" + time + "</div>").attr({
			"id": oid + "-div3",
			"class": "col-md-3 col-sm-3"
		}));
		// div4
		$("#" + oid + "-div2").append($("<div>" + orderNo + "</div>").attr({
			"id": oid + "-div4",
			"class": "col-md-3 col-sm-3"
		}));
		// div5
		$("#" + oid + "-div2").append($("<div>数量</div>").attr({
			"id": oid + "-div5",
			"class": "col-md-1 col-sm-1"
		}));
		// div6
		$("#" + oid + "-div2").append($("<div>收货人</div>").attr({
			"id": oid + "-div6",
			"class": "col-md-1 col-sm-1"
		}));
		// div7
		$("#" + oid + "-div2").append($("<div>金额</div>").attr({
			"id": oid + "-div7",
			"class": "col-md-1 col-sm-1"
		}));
		// div8
		$("#" + oid + "-div2").append($("<div>状态</div>").attr({
			"id": oid + "-div8",
			"class": "col-md-1 col-sm-1"
		}));
		// div9
		$("#" + oid + "-div2").append($("<div></div>").attr({
			"id": oid + "-div9",
			"class": "col-md-1 col-sm-1"
		}));
		// btn0
		if (status == 0) {
			$("#" + oid + "-div9").append($("<button>" + "去付款" + "</button>").attr({
				"id": oid + "-btn0",
				"name": "s0",
				"type": "button",
				"class": "btn btn-danger btn-xs",
				"onclick": "payOrRefund(" + "\"" + btn0Id + "\"" + ")"
			}));
		} else if (status == 1) {
			$("#" + oid + "-div9").append($("<button>" + "退款" + "</button>").attr({
				"id": oid + "-btn0",
				"name": "s1",
				"type": "button",
				"class": "btn btn-danger btn-xs",
		...
		// btn1
		if (status == 0) {
			$("#" + oid + "-div10").append($("<button>" + "取消订单" + "</button>").attr({
				"id": oid + "-btn1",
				"type": "button",
				"class": "btn btn-danger btn-xs",
				"onclick": "cancelOrder(" + "\"" + btn1Id + "\"" + ")"
			}));
		}
		...
		for (; orderJson.data.list[i].orderItemList[j]; j++) {
			...
			// div1
			$("#" + oiid + "-div0").append($("<div></div>").attr({
				"id": oiid + "-div1",
				"class": "col-md-2 col-sm-2"
			}));
			// img0
			$("#" + oiid + "-div1").append($("<img></img>").attr({
				"id": oiid + "-img0",
				"src": imgSrc,
				"width": "100px"
			}));
			...
			$("#" + oiid + "-div0").append($("<div></div>").attr({
				"id": oiid + "-div2",
				"class": "col-md-4 col-sm-4 margin-top-20"
			}));
			// strong0
			$("#" + oiid + "-div2").append($("<strong>" + name + "</strong>"));
			// div3
			$("#" + oiid + "-div0").append($("<div>" + "x" + quantity + "</div>").attr({
				"id": oiid + "-div3",
				"class": "col-md-1 col-sm-1 margin-top-20"
			}));
			// div4
			$("#" + oiid + "-div0").append($("<div>" + shippingName + "</div>").attr({
				"id": oiid + "-div4",
				"class": "col-md-1 col-sm-1 margin-top-20"
			}));
			// div5
			$("#" + oiid + "-div0").append($("<div>" + totalPrice + "</div>").attr({
				"id": oiid + "-div5",
				"class": "col-md-1 col-sm-1 margin-top-20"
			}));
			// div6
			$("#" + oiid + "-div0").append($("<div>" + itemStatus + "</div>").attr({
				"id": oiid + "-div6",
				"class": "col-md-2 col-sm-2 margin-top-20"
			}));
		}
	}
}
