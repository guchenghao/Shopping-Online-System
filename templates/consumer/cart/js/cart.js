...
function showCartList() {
	...
	for (; cartJson.data.cartGoodsBoList[i]; i++) {
		...
		// 0-div0
		$("#start").after($("<div></div>").attr({
			"id": cid + "-div0",
			"class": "row"
		}));
		// 0-div1
		$("#" + cid + "-div0").after($("<div></div>").attr({
			"id": cid + "-div1",
			"class": "col-md-1 col-sm-1"
		}));
		// 0-div2
		$("#" + cid + "-div1").after($("<div></div>").attr({
			"id": cid + "-div2",
			"class": "col-md-2 col-sm-2"
		}));
		// 0-input0
		$("#" + cid + "-div2").append($("<input></input>").attr({
			"id": cid + "-input0",
			"name": "singleChk",
			"type": "checkbox",
			"onclick": "checkboxEvt(" + "\"" + input0Id + "\"" + ")"
		}));
		if (selected == 0)
			$("#" + cid + "-input0").prop("checked", true);
		// 0-img0
		$("#" + cid + "-div2").append($("<img></img>").attr({
			"id": cid + "-img0",
			"src": imgSrc,
			"width": "100px"
		}));
		// 0-div3
		$("#" + cid + "-div2").after($("<div></div>").attr({
			"id": cid + "-div3",
			"class": "col-md-3 col-sm-3 margin-top-20"
		}));
		$("#" + cid + "-div3").append($("<strong>" + name + "</strong>"));
		// 0-div4
		$("#" + cid + "-div3").after($("<div></div>").attr({
			"id": cid + "-div4",
			"class": "col-md-1 margin-top-20"
		}));
		// 0-div5
		$("#" + cid + "-div4").append($("<div>" + price + "</div>").attr({
			"id": cid + "-div5",
			"class": "pull-right"
		}));
		// 0-div6
		$("#" + cid + "-div4").after($("<div></div>").attr({
			"id": cid + "-div6",
			"class": "col-md-2 col-sm-2 margin-top-20"
		}));
		// 0-div7
		$("#" + cid + "-div6").append($("<div></div>").attr({
			"id": cid + "-div7",
			"class": "input-group input-group-sm col-md-offset-4"
		}));
		// 0-span0
		$("#" + cid + "-div7").append($("<span></span>").attr({
			"id": cid + "-span0",
			"class": "input-group-btn"
		}));
		// 0-btn0
		$("#" + cid + "-span0").append($("<button>-</button>").attr({
			"id": cid + "-btn0",
			"type": "button",
		...
		// 0-input1
		$("#" + cid + "-div7").append($("<input></input>").attr({
			"id": cid + "-input1",
			"type": "text",
			"value": quantity,
			"class": "form-control",
		...
		// 0-btn1
		$("#" + cid + "-span1").append($("<button>+</button>").attr({
			"id": cid + "-btn1",
			"type": "button",
			"class": "btn btn-default",
			"onclick": "add(" + "\"" + btn1Id + "\"" + ")"
		...
		// 0-div8
		$("#" + cid + "-div6").after($("<div>" + subTotalPrice + "</div>").attr({
			"id": cid + "-div8",
			"class": "col-md-1 margin-top-20"
		}));
		// 0-div9
		$("#" + cid + "-div8").after($("<div></div>").attr({
			"id": cid + "-div9",
			"class": "col-md-1 margin-top-20"
		}));
		// 0-a0
		$("#" + cid + "-div9").append($("<a>" + "删除" + "</a>").attr({
			"id": cid + "-a0",
			"href": href
		}));
		// 0-div10
		$("#" + cid + "-div9").after($("<div></div>").attr({
			"id": cid + "-div10",
			"class": "col-md-1"
		}));
	}
