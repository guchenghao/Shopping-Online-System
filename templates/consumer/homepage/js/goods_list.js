...
function showGoodsList() {
	...
	for (; goodsListJson.data.list[i]; i++) {
		...
		// 0-div1
		$("#" + gid + "-div0").append($("<div></div>").attr({
			"id": gid + "-div1",
			"class": "thumbnail"
		}));
		// 0-img0
		$("#" + gid + "-div1").append($("<img></img>").attr({
			"id": gid + "-img0",
			"src": imgSrc
		}));
		// 0-div2
		$("#" + gid + "-div1").append($("<div></div>").attr({
			"id": gid + "-div2",
			"class": "caption"
		}));
		// 0-p0
		$("#" + gid + "-div2").append($("<p></p>").attr({
			"id": gid + "-p0"
		}));
		// 0-a0
		$("#" + gid + "-p0").append($("<a>" + name + "</a>").attr({
			"id": gid + "-a0",
			"href": href
		}));
		// 0-h30
		$("#" + gid + "-div2").append($("<h3></h3>").attr({
			"id": gid + "-h30",
			"style": "color:red"
		}));
		// 0-strong0
		$("#" + gid + "-h30").append($("<strong>" + "¥" + price + "</strong>").attr({
			"id": gid + "-strong0"
		}));
	}
}
